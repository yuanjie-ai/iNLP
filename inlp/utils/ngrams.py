...
class Ngrams(object):
    ...
    def skipgrams(self, sequence, n, k, **kwargs):
        """
        ...
        if 'pad_left' in kwargs or 'pad_right' in kwargs:
            sequence = self.pad_sequence(sequence, n, **kwargs)
        ...
        SENTINEL = object()
        for ngram in self.ngrams(sequence, n + k, pad_right=True, right_pad_symbol=SENTINEL):
            head = ngram[:1]
            tail = ngram[1:]
